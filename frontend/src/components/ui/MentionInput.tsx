    ...
    const highlightedContent = useMemo(() => {
        ...
        while ((match = regex.exec(value)) !== null) {
            ...
            parts.push(
                <span
                    key={`m-${matchStart}`}
                    className={isValidFile ? "text-sky-600 font-medium" : "text-gray-500"}
                >
                    {match[0]}
                </span>
            );
        ...
        <div className="relative flex-1">
            <div className="relative">
                ...
                <textarea
                    ...
                    disabled={disabled}
                    className={`w-full border-2 border-sky-200 rounded-xl p-3 resize-none min-h-[48px] max-h-[120px] focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent transition-all text-sm bg-white placeholder:text-gray-400 ${className}`}
                    style={{
                        caretColor: '#0284c7',
                        lineHeight: '1.5',
            ...
            {showDropdown && filteredFiles.length > 0 && (
                <div
                    ref={dropdownRef}
                    className="absolute bottom-full left-0 mb-1 w-full max-h-48 overflow-y-auto bg-white rounded-lg shadow-lg border-2 border-sky-200 z-50"
                >
                    ...
                    {filteredFiles.map((file, index) => (
                        <button
                            ...
                            className={`w-full flex items-center gap-2 px-3 py-2 text-left text-sm hover:bg-sky-50 transition-colors ${index === dropdownIndex ? "bg-sky-50" : ""}`}
                        >
            ...
            {showDropdown && filteredFiles.length === 0 && (
                <div
                    ref={dropdownRef}
                    className="absolute bottom-full left-0 mb-1 w-full bg-white rounded-lg shadow-lg border-2 border-sky-200 z-50 p-3"
                >
