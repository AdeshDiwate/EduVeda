      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8">
        ...
        <div className="mb-6 sm:mb-8 text-center">
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-semibold text-gray-800 mb-2 sm:mb-3">
            Generate <span className="text-sky-400">Educational Posters</span>
          </h1>
        ...
        <div className="mb-6 sm:mb-8">
          <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border-2 border-sky-200/60 p-5 sm:p-6 md:p-8">
            ...
            <div className="relative mb-4">
              <textarea
                ...
                rows={4}
                maxLength={500}
                className="w-full px-4 py-3 bg-white border-2 border-sky-200 rounded-xl text-sm sm:text-base resize-none focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent text-gray-700 transition-all"
              />
              <div className="absolute bottom-3 right-3">
                <span className={`text-xs ${prompt.length > 450 ? 'text-sky-500' : 'text-gray-500'}`}>
                  {prompt.length}/500
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-4 sm:gap-6 mb-6 sm:mb-8">
          {/* Category Selector */}
          <div className="lg:col-span-2 bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border-2 border-sky-200/60 p-4 sm:p-5">
              ...
              {categories.map((category) => (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg text-xs sm:text-sm font-medium transition-all ${selectedCategory === category.id
                      ...
                      : 'bg-white text-gray-700 border-2 border-sky-200 hover:bg-sky-50 hover:border-sky-300'
                    }`}
                >
          ...
          {/* Language Selector */}
          <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border-2 border-sky-200/60 p-4 sm:p-5">
            ...
            <select
              ...
              className="w-full px-3 py-2.5 bg-white border-2 border-sky-200 rounded-lg text-xs sm:text-sm focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent text-gray-700 transition-all"
            >
              {languages.map((lang) => (
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
              ...
              <button
                ...
                className={`px-6 sm:px-8 py-3 sm:py-3.5 rounded-full font-semibold text-sm sm:text-base shadow-md transition-all transform w-full sm:w-auto ${!prompt.trim() || isGenerating
                    ...
                    : 'bg-sky-400 text-white hover:bg-sky-500 hover:shadow-lg hover:scale-105'
                  }`}
              >
                {isGenerating ? (
          ...
          {isGenerating ? (
            ...
            <div className="p-6 sm:p-8 md:p-10">
              <div className="flex items-center justify-between mb-6">
                ...
                <div className="hidden sm:flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                  <span className="px-2.5 py-1 bg-sky-50 border border-sky-200 rounded-full">{categories.find(c => c.id === selectedCategory)?.name}</span>
              ...
              <div className={`grid ${getGridClass()} gap-4 sm:gap-6 mb-8`}>
                {mockImages.map((image) => (
                  <div key={image.id} className="group relative overflow-hidden rounded-xl border-2 border-sky-200 shadow-md hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                    <div className="aspect-video bg-gradient-to-br from-sky-100 to-sky-200 flex items-center justify-center">
                      <div className="text-center">
                        <svg className="w-12 h-12 sm:w-16 sm:h-16 text-sky-400 mx-auto mb-2" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                          <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"></path>
                        </svg>
                        <p className="text-xs sm:text-sm text-sky-600 font-medium">Poster {image.id}</p>
                      </div>
                    </div>
                    <div className="absolute inset-0 bg-gradient-to-t from-black/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity flex items-end p-4">
                      <div className="w-full">
                        ...
                        <p className="text-sky-200 text-xs sm:text-sm mb-3">Click to download</p>
                        <button className="w-full px-4 py-2 bg-sky-400 text-white rounded-lg font-medium text-xs sm:text-sm hover:bg-sky-500 transition-all">
                          Download
                        </button>
                      </div>
                    </div>
            ...
            <div className="text-center py-16 sm:py-20">
              ...
              <h3 className="text-xl sm:text-2xl md:text-3xl font-semibold text-gray-800 mb-3">
                <span className="text-sky-400">Enter Prompt</span> to Generate Posters
              </h3>
