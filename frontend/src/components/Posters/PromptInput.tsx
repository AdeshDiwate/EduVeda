...
const PromptInput: React.FC<PromptInputProps> = ({ 
  ...
  return (
    <div className="bg-white/80 backdrop-blur-md rounded-2xl p-5 sm:p-6 shadow-xl border-2 border-sky-200/60">
      ...
      <div className="mb-4">
        <textarea
          ...
          className="w-full border border-gray-200 rounded-lg p-4 resize-none h-32 focus:ring-2 focus:ring-sky-200 focus:border-sky-300 bg-white text-gray-900 placeholder-gray-400 transition-all"
          maxLength={500}
        />
        ...
        <div className="flex flex-wrap gap-2">
          {getExamplePrompts().slice(0, 3).map((example, index) => (
            <button
              ...
              className="text-xs px-3 py-1.5 bg-sky-50 text-sky-700 rounded-full hover:bg-sky-100 transition-colors border border-sky-200"
            >
      ...
      <button
        ...
        className={`w-full px-6 py-3 rounded-xl font-semibold text-base transition-all ${
          (!prompt.trim() || isGenerating)
            ...
            : 'bg-linear-to-r from-sky-500 to-sky-600 text-white hover:from-sky-600 hover:to-sky-700 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5'
        }`}
      >
        {isGenerating ? (
