...
const TreeNodeComponent: React.FC<TreeNodeProps> = ({
    ...
    return (
        <div className={`${depth > 0 ? 'ml-6 border-l-2 border-sky-100' : ''}`}>
            <div
                className={`
                    group relative flex items-start gap-3 p-3 rounded-xl transition-all duration-200
                    bg-white hover:bg-sky-50/50
                    ${depth === 0 ? 'shadow-sm border-2 border-sky-100 hover:border-sky-200' : ''}
                `}
            >
                ...
                {hasChildren && (
                    <button
                        ...
                        className="flex-shrink-0 w-6 h-6 flex items-center justify-center rounded-md bg-sky-100 hover:bg-sky-200 text-sky-600 transition-colors"
                    >
                        <svg
