...
const MasterPlanView: React.FC<MasterPlanViewProps> = ({ userId, sessionId }) => {
    ...
    if (!plan && !isLoading) {
        return (
            <div className="bg-white rounded-xl border border-sky-200 p-6 mb-8 shadow-sm">
                <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                    ...
                    <button
                        ...
                        className="px-6 py-2 bg-gradient-to-r from-sky-500 to-amber-500 text-white rounded-lg hover:from-sky-600 hover:to-amber-600 font-medium shadow-md transition-all whitespace-nowrap"
                    >
                        Generate Study Plan
                    </button>
                </div>
    ...
    if (isLoading && !plan) {
        return (
            <div className="bg-white rounded-xl border border-sky-200 p-8 mb-8 shadow-sm text-center">
                <div className="w-8 h-8 mx-auto mb-3 border-4 border-sky-200 border-t-sky-500 rounded-full animate-spin" />
    ...
    return (
        <div className="bg-white rounded-xl border border-sky-200 overflow-hidden mb-8 shadow-sm">
            {/* Header */}
            <div className="p-4 bg-gradient-to-r from-sky-50 to-amber-50 border-b border-sky-100 flex items-center justify-between flex-wrap gap-4">
                <div
                    className="flex items-center gap-2 cursor-pointer"
                    onClick={() => setIsExpanded(!isExpanded)}
                >
                    <svg
                        className={`w-5 h-5 text-sky-500 transition-transform ${isExpanded ? 'rotate-90' : ''}`}
                        fill="none"
                        stroke="currentColor"
                        viewBox="0 0 24 24"
                    >
                ...
                {isExpanded && (
                    <div className="flex items-center gap-2">
                        <select
                            ...
                            className="text-sm border border-gray-300 rounded-lg px-2 py-1.5 focus:border-sky-500 focus:ring-1 focus:ring-sky-500 outline-none"
                        >
