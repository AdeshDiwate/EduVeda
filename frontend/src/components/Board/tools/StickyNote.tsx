...
const StickyNote: React.FC<StickyNoteProps> = ({
  ...
  return (
    <div
      ref={noteRef}
      className={`absolute select-none ${selectionMode ? 'cursor-move' : ''} ${isSelected ? 'ring-2 ring-sky-500 ring-offset-2' : ''}`}
          ...
          <div className="space-y-3">
            ...
            <div>
              <label className="flex items-center justify-between text-xs text-gray-700 mb-1">
                ...
                <button
                  className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${ruled ? 'bg-sky-500' : 'bg-gray-300'
                    }`}
                  onClick={(e) => {
                    e.stopPropagation();
