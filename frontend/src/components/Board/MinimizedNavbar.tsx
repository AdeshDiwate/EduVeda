  ...
  return (
    <>
      ...
      {!isExpanded && (
        <button
          onClick={onToggle}
          className="fixed left-4 top-1/2 -translate-y-1/2 z-40 w-11 h-11 rounded-full bg-white/90 border border-sky-200 shadow-lg transition-all duration-300 flex items-center justify-center hover:-translate-x-0.5 hover:shadow-xl"
          title="Expand Menu"
        >
          <span className="text-sm font-semibold tracking-tight text-sky-500">MG</span>
        </button>
      )}
      ...
      {isExpanded && (
        <>
          {/* Side Panel */}
          <div className="fixed left-5 top-1/2 -translate-y-1/2 w-64 max-h-[80vh] bg-white/95 backdrop-blur-xl shadow-2xl z-50 transform transition-transform duration-300 flex flex-col rounded-2xl border border-sky-100/80 overflow-hidden">
            {/* Header */}
            <div className="flex items-center justify-between px-4 py-3 border-b border-sky-100/80 bg-gradient-to-r from-sky-50/80 via-white to-sky-50/40">
              <a
                href="/"
                className="text-xl font-semibold tracking-tight text-sky-500 hover:text-sky-600 transition-colors"
                onClick={(e) => {
                  e.preventDefault();
                  navigate('/');
                  onToggle();
              ...
              <button
                onClick={onToggle}
                className="text-sky-400 hover:text-sky-600 transition-colors p-1.5 hover:bg-sky-50 rounded-lg"
                title="Collapse Menu"
              >
                <svg
                  className="w-6 h-6"
            ...
            <nav className="flex-1 px-3 py-3 space-y-1.5 overflow-y-auto">
              <a
                ...
                className={`flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm transition-all ${isActive('/stitch')
                  ...
                  : 'text-sky-600 hover:bg-sky-50/80 hover:text-sky-700'
                  }`}
                onClick={(e) => {
                  e.preventDefault();
                  navigate('/stitch');
            ...
            {/* Footer */}
            <div className="px-4 py-3 border-t border-sky-100/80 bg-gradient-to-r from-white to-sky-50/60 space-y-2">
              <a
                href="https://github.com/ayuxy027/EduVeda"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center justify-center w-full h-10 text-sky-600 hover:text-sky-700 hover:bg-sky-50/80 rounded-xl transition-all"
                onClick={onToggle}
              ...
              <a
                href="/chat"
                className="block w-full bg-sky-500 text-white px-4 py-2 rounded-full text-sm font-medium hover:bg-sky-600 transition-all text-center shadow-md hover:shadow-lg"
                onClick={(e) => {
                  e.preventDefault();
                  navigate('/chat');
                  onToggle();
