...
const CanvasDock: React.FC<CanvasDockProps> = ({
  ...
  return (
    <div className={`fixed bottom-6 left-1/2 -translate-x-1/2 z-50 transition-all duration-300 ${sidebarOpen ? 'ml-20' : 'ml-2.5'}`}>
      <div className="rounded-2xl bg-white/95 backdrop-blur-xl shadow-xl border border-sky-200/60 px-4 py-3">
        <div className="flex items-center gap-6">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="relative" ref={generatePanelRef}>
              ...
              {/* Generate Panel */}
              {isGeneratePanelOpen && (
                <div className="absolute bottom-14 left-0 w-80 bg-white rounded-xl shadow-xl border border-sky-200 p-4 z-50">
                  ...
                  <form onSubmit={handleGenerate} className="space-y-3">
                    <textarea
                      ...
                      className="w-full px-3 py-2 border-2 border-sky-200 rounded-lg resize-none focus:border-sky-400 focus:outline-none"
                      rows={3}
          ...
          <div className="flex items-center gap-4">
            <div className="flex items-center gap-2">
              <LuPalette size={16} className="text-sky-500" />
              <button
                onClick={() => colorInputRef.current?.click()}
                className="w-6 h-6 rounded-full border-2 border-sky-300 shadow-sm hover:scale-110 transition-transform"
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="text-sm text-sky-600 font-medium w-5">{strokeWidth}</span>
            </div>
          </div>
          ...
          <div className="flex items-center gap-1">
            ...
            <span className="text-sm text-sky-600 font-medium w-12 text-center">
              {Math.round(zoom * 100)}%
            </span>
            <button
          ...
          <div className="flex items-center gap-1 relative" ref={sessionsPanelRef}>
            ...
            <button
              onClick={onNewBoard}
              className="flex flex-col items-center gap-1 px-2 py-1 text-sky-600 hover:bg-sky-50 rounded-lg transition-colors"
              title="New Board"
            >
