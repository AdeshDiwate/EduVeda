...
const RecallNotesTab: React.FC<RecallNotesTabProps> = ({
  ...
  return (
    <div className="space-y-6">
      {/* Header */}
      <div className="bg-gradient-to-r from-sky-50 to-sky-100 border-2 border-sky-200 rounded-xl p-6">
      ...
      {recallNotes.map((note, index) => {
        ...
        return (
          <div
            key={index}
            className="bg-white rounded-xl border-2 border-sky-200 overflow-hidden hover:shadow-lg transition-all duration-300"
          >
            {/* Topic header */}
            <div className="bg-sky-50 border-b-2 border-sky-200 px-6 py-4">
                ...
                <div>
                  ...
                  <ul className="space-y-2">
                    {keyPoints.map((point, idx) => (
                      <li
                        key={idx}
                        className="flex items-start gap-3 p-3 bg-sky-50 rounded-lg border border-sky-200"
                      >
                        <span className="flex-shrink-0 w-6 h-6 bg-sky-400 text-white rounded-full flex items-center justify-center text-xs font-bold">
                          {idx + 1}
                        </span>
