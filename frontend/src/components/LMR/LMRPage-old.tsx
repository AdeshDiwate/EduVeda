      ...
      <div className="max-w-[1600px] mx-auto px-4 sm:px-6 md:px-8 py-6 sm:py-8">
        ...
        <div className="mb-6 sm:mb-8 text-center">
          <h1 className="text-2xl sm:text-3xl md:text-4xl lg:text-5xl font-semibold text-gray-800 mb-2 sm:mb-3">
            <span className="text-sky-400">Last Minute Recall</span> (LMR)
          </h1>
        ...
        <div className="flex flex-col lg:flex-row gap-4 sm:gap-6 items-stretch">
          ...
          <aside className="lg:w-80 xl:w-96 flex-shrink-0">
            <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border-2 border-sky-200/60 p-5 sm:p-6 sticky top-24 space-y-6 h-full">
              {/* Upload Section */}
              <div>
                ...
                <div 
                  className={`border-2 border-dashed rounded-xl p-5 sm:p-6 text-center cursor-pointer transition-all ${
                    uploadedFile ? 'border-sky-400 bg-sky-50' : 'border-sky-200 hover:border-sky-400 hover:bg-sky-50'
                  }`}
                  onClick={() => document.getElementById('file-upload')?.click()}
                >
                  {!uploadedFile ? (
                    <>
                      <svg className="w-12 h-12 sm:w-14 sm:h-14 text-sky-400 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M7 16a4 4 0 01-.88-7.903A5 5 0 1115.9 6L16 6a5 5 0 011 9.9M15 13l-3-3m0 0l-3 3m3-3v12"></path>
                      </svg>
                    ...
                    <>
                      <svg className="w-12 h-12 sm:w-14 sm:h-14 text-sky-500 mx-auto mb-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z"></path>
                      </svg>
                      <p className="text-sm text-gray-800 font-medium break-words mb-1">{uploadedFile.name}</p>
                      <p className="text-xs text-sky-600 font-medium">Uploaded successfully</p>
                    </>
                  )}
                  ...
                  {isProcessing && (
                    <div className="mt-4">
                      <div className="w-full bg-sky-200 rounded-full h-2.5">
                        <div className="bg-sky-500 h-full rounded-full animate-pulse w-3/4 transition-all"></div>
                      </div>
                      <p className="text-xs text-sky-600 mt-2 font-medium">Processing document...</p>
                    </div>
                  )}
                </div>
                <input
              ...
              <div>
                ...
                <select 
                  ...
                  className="w-full px-3 py-2.5 bg-white border-2 border-sky-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-sky-400 focus:border-transparent text-gray-700 transition-all"
                >
                  {languages.map((lang) => (
              ...
              {/* Status Indicator */}
              {hasContent && (
                <div className="bg-sky-50 border-2 border-sky-200 rounded-lg p-3.5">
          ...
          <main className="flex-1 min-w-0 flex flex-col">
            <div className="bg-white/80 backdrop-blur-md rounded-2xl shadow-xl border-2 border-sky-200/60 overflow-hidden flex-1 flex flex-col">
              {/* View Navigation Dock */}
              <div className="bg-sky-100 border-b-2 border-sky-200 p-3 sm:p-4 md:p-5">
                      ...
                      <button
                        ...
                        className={`flex items-center gap-1.5 sm:gap-2 px-3 sm:px-4 md:px-5 py-2 sm:py-2.5 rounded-full transition-all font-semibold text-xs sm:text-sm md:text-base shadow-md transform whitespace-nowrap flex-shrink-0 ${
                          isDisabled
                            ...
                            : activeView === view.id
                            ? 'bg-sky-400 text-white hover:bg-sky-500 shadow-lg hover:scale-105'
                            : 'bg-white text-sky-600 border-2 border-sky-200 hover:bg-sky-50 hover:border-sky-300 hover:scale-105'
                        }`}
                      >
              ...
              <div className="p-6 sm:p-8 md:p-10 min-h-[500px]">
                {!hasContent ? (
                  <div className="text-center py-16 sm:py-20">
                    <div className="w-20 h-20 sm:w-24 sm:h-24 mx-auto mb-6 rounded-full bg-sky-100 flex items-center justify-center">
                      <svg className="w-10 h-10 sm:w-12 sm:h-12 text-sky-400" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path>
              </svg>
                    </div>
                    <h3 className="text-xl sm:text-2xl md:text-3xl font-semibold text-gray-800 mb-3">
                      <span className="text-sky-400">Upload Document</span> to Get Started
                    </h3>
                  ...
                  <>
                      ...
                      <div>
                        <div className="flex items-center justify-between mb-6">
                          ...
                          <div className="hidden sm:flex items-center gap-2 text-xs sm:text-sm text-gray-600">
                            <span className="px-2.5 py-1 bg-sky-50 border border-sky-200 rounded-full">{languages.find(l => l.value === selectedLanguage)?.label}</span>
                            <span className="px-2.5 py-1 bg-sky-50 border border-sky-200 rounded-full">{tones.find(t => t.value === selectedTone)?.label}</span>
                          </div>
                        </div>
                        <MaterialsTab 
                    ...
                    {activeView === 'questions' && (
                      <div>
                        <div className="flex items-center justify-between mb-6">
                          <div className="flex items-center gap-3">
                            <svg className="w-6 h-6 text-sky-400" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg">
                              <path d="M3.85 8.62a4 4 0 0 1 4.78-4.77 4 4 0 0 1 6.74 0 4 4 0 0 1 4.78 4.78 4 4 0 0 1 0 6.74 4 4 0 0 1-4.77 4.78 4 4 0 0 1-6.75 0 4 4 0 0 1-4.78-4.77 4 4 0 0 1 0-6.76Z"/>
                              <path d="M9.09 9a3 3 0 0 1 5.83 1c0 2-3 3-3 3"/>
                              <line x1="12" x2="12.01" y1="17" y2="17"/>
                            </svg>
                    ...
                    {activeView === 'quiz' && (
                      <div>
                        ...
                        <div className="bg-sky-50 border-2 border-sky-200 rounded-xl p-8 text-center">
                    ...
                    {activeView === 'pdf' && (
                      <div>
                        ...
                        <div className="bg-sky-50 border-2 border-sky-200 rounded-xl p-8 text-center">
                          <button className="px-8 py-4 bg-sky-400 text-white rounded-xl hover:bg-sky-500 transition-all shadow-lg hover:shadow-xl transform hover:scale-105 font-semibold text-base sm:text-lg flex items-center gap-2 mx-auto">
